...
⚛️ SAPFix AI uses React `18.3.1`, React DOM `18.3.1`, TypeScript `5.6.3`, and Vite `5.4.11` on the frontend to provide the chat interface, sidebar navigation, and model-selection workflow. 🐍 The backend is built with Python, FastAPI `0.115.6`, and Uvicorn `0.34.0`, while the OpenAI SDK `1.68.2`, Requests `2.32.3`, and Python Dotenv `1.0.1` handle provider integration, HTTP communication, and local environment configuration.

🧠 For model support, the project currently uses GitHub Models with `openai/gpt-4o-mini` as the hosted GPT path and Ollama with `llama3.1:8b` as the locally installed LLM path, with a local knowledge-base synthesis fallback when generation is unavailable. 🗃️ On the data and retrieval side, the repository includes Pandas `2.2.2`, OpenPyXL `3.1.5`, Sentence Transformers `3.0.1`, Transformers `4.44.0`, PyTorch `2.3.1+cu121`, and ChromaDB `0.5.5`.

🛠️ For development and local execution, the workflow uses `npm` for the frontend, `pip` for Python dependencies, and local `.env` configuration for provider settings. 🔎 The current API path uses an offline-safe keyword retrieval flow over the processed SAP knowledge-base JSON, while the legacy embedding and vector-store modules remain in the repo for future retrieval upgrades.
